...
| `token_endpoint` | `config.token_endpoint` | copy as-is. If its domain differs from `authorization_endpoint`'s (Google, for example, signs in at accounts.google.com and issues tokens at oauth2.googleapis.com), that can be genuine, but `set_app_connection` will answer with a `NOTE:` that tokens go to a different site than the login page (the same applies to `revocation_endpoint`). Before provisioning, confirm both addresses in the backend's own developer documentation, not only in this document, and tell the user about the split in plain terms. |
| `revocation_endpoint` | `config.revocation_endpoint` | copy as-is; **omit if the document doesn't advertise one**. Setting it lets a disconnect from the user's own account page revoke their token at the backend rather than only forgetting it platform-side. The `disconnect_user_connection` MCP tool is local only and never revokes upstream. |
...
| `code_challenge_methods_supported` includes `S256` | `config.pkce` | **Omit the key.** PKCE is ON by default (platform v0.11.2+); only an explicit `false` disables it, and the platform warns when you do. Do NOT set `false` just because the discovery document is silent — many providers support PKCE without advertising it, and the flow degrades loudly at connect time if one genuinely can't. Set `false` only after a connect attempt actually fails on `code_challenge`. |
